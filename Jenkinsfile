      ...
      stage('Kubernetes Deployment - DEV') {
            steps {
              ...
              //withKubeConfig([ credentialsId: "kubeconfig"]) {
                ...
                sh 'kubectl --kubeconfig config apply -f k8s_deployment_service.yaml -n jenkins --validate=false'
                sh 'kubectl --kubeconfig config create deploy node-app --image siddharth67/node-service:v1 -n jenkins'
                sh 'kubectl --kubeconfig config expose deploy node-app --name node-service --port 5000 -n jenkins'
              }
        //}
      }
    }
